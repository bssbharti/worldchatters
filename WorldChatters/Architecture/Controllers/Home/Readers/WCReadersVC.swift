...
class WCReadersVC: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == SegueIdentity.kReaderProfileSegue ,let btn = sender as? JKButton
        {
            let controller = segue.destination as! WCReaderProfileVC
            controller.readerViewModel = readerViewModel
            controller.selectedIndexPath =  IndexPath(row: btn.tag, section: 0)
